...
A configuration-driven AI training system that orchestrates the full machine learning lifecycle: dataset preparation, model training, evaluation, fine-tuning, post-processing, experiment tracking, with containerized execution and GPU support.
...
- Configuration-driven ML pipelines (Pydantic schemas)
- Dataset preparation (train/val/test splits, transforms, metadata)
- DAGNet support
- Vary of manually written layers to enrich network(e.g. Linear, Conv, Transformers..) 
...
- Evaluation of models with included error analysis tables, confusion matrix & metrics
...
- **FastAPI Backend**  
  Acts as the main entry point for users. It exposes endpoints to trigger dataset preparation, model training, evaluation, and fine-tuninf and post-processing jobs. All requests are validated against structured configuration schemas.
...
- **Experiment Tracking (MLflow)**  
  All runs, metrics, artifacts, datasets, trained models, error tables, confussion matrices are logged to MLflow. This enables experiment comparison, reproducibility, and model versioning.
