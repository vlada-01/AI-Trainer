...
class TextualDataMeta(DataMeta):
    def __init__(self, specs_mapping):
        super().__init__(MetaTypes.textual, specs_mapping)
        self.vocab = None
        self.max_len = None
        ...
        for k, v in upd_dict.items():
            if hasattr(self, k):
                if callable(getattr(self, k)):
                    log.info(f'Calling TextualDataMeta attr "{k}"')
                    fn = getattr(self, k)
                    ...
                    raise ValueError(f'TextualDataMeta does not have callable: {k}')
            else:
                log.warning(f'TextualDataMeta does not have attr {k}')
    ...
    def set_output_unique_values(self, train_ds):
        log.info('Updating TextualDataMeta sizes')
