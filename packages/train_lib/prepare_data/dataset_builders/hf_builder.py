...
class HuggingFaceBuilder():
    def __init__(self, cfg, cfg_dataset_transforms, meta):
        self.cfg = cfg
        ...
        raw = self.preprocess_data(raw, meta)
        ...
        update_meta(meta, upd_dict)

        log.info('Assembling dataset transformations')
        train_t, train_tt, val_t, val_tt, test_t, test_tt = assemble_transforms(self.cfg_dataset_transforms, meta)
    ...
    def preprocess_data(self, raw, meta):
        x_keys = self.cfg.x_keys
        y_keys = self.cfg.y_keys
        specs_mapper = meta.specs_mapper
        ...
        def build_xy_features(raw_features):
            x = {specs_mapper(k.split('.')[-1]): get_by_path(raw_features, k) for k in x_keys}
            y = {specs_mapper(k.split('.')[-1]): get_by_path(raw_features, k) for k in y_keys}
            return Features({'x': x, 'y': y})
        
        def normalize_inputs(batch):
            xs_dict = {specs_mapper(k.split('.')[-1]): get_by_path(batch, k) for k in x_keys}
            ys_dict = {specs_mapper(k.split('.')[-1]): get_by_path(batch, k) for k in y_keys}
